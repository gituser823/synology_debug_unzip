...
def analyze(debug_dir: Path, download_dir: Path, sm_prefix: str = ""):
    ...
    first_party = (
        ...
        r"ActiveInsight|ContainerManager|SMBService|SupportService"
    )
    third_pkgs = [
...
def install_sublime_packages():
    ...
    lh_gen = SCRIPT_DIR / "sublime" / "gen_loghighlight_syntax.py"
    if lh_gen.exists():
        spec = importlib.util.spec_from_file_location("gen_lh", lh_gen)
        gen_mod = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(gen_mod)
        gen_mod.gen_loghighlight_syntax(settings_dir, lh_settings_src)
