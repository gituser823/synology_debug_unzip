...
def update_compatibility_lists():
    ...
    try:
        ...
        skip_prefixes = ("PSU ", "ioSafe ", "Power Cord", "FAN ", "Disk Tray", "Drive Tray", "Adapter ", "CPU ", "Cable ", "2.5")
...
def install_sublime_packages():
    ...
    # Pre-generate tmLanguage + tmTheme so Sublime highlights sm.log immediately
    # without requiring "Log Highlight: Generate Syntax & Theme" from the command palette.
    _gen_loghighlight_syntax(settings_dir, lh_settings_src)


def _gen_loghighlight_syntax(settings_dir: Path, lh_settings_src: Path):
    """Generate Packages/User/Log Highlight/sm_log-log.{tmLanguage,tmTheme}."""
    ...
    for log_name, log_cfg in cfg.get("log_list", {}).items():
        ...
        tmlang = (f'<?xml version="1.0" encoding="UTF-8"?>\n'
        ...
        for k in svl:
            for c, v in svt[k].get("color", {}).items():
                p = "" if c == "base" else "." + c
                fg = f'\n                <key>foreground</key><string>{v[0]}</string>' if isinstance(v, list) else f'<key>foreground</key><string>{v}</string>'
                bg = (f'\n                <key>background</key><string>{v[1]}</string>' if isinstance(v, list) and v[1] else "")
                sub_theme += (f'\n        <dict>'
