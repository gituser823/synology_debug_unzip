...
if grep -qE "(Microsoft|WSL)" /proc/version &> /dev/null ; then
    ...
    OpenFiles=("${DEBUG_DIR}" "${SMART_GREP}" "${PACK}" "${Bash_history}" "${hb_debug}" "${HB}" "${DF}" "${IFCONFIG}" "${SPACE_FILES}":100000 "${DiskLog}" "${SYSDBtac}":100000 "${sm}" "${MESSAGES}":1000000 "${PicArray[@]}") # for subl
